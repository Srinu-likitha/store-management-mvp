...
export default function AddMaterialInvoice() {
  ...
  const materialInvoiceMutation = useMutation({
    mutationFn: async (data: MaterialInvoiceInput) => {
      const res = await api.post<Response>(API_ROUTES.MATERIAL_INVOICES.CREATE_MATERIAL_INVOICE, data);
      return res.data;
    },
    onSuccess: () => {
  ...
  const onSubmit = (data: MaterialInvoiceInput) => {
    setIsSubmitting(true);
    materialInvoiceMutation.mutate(data);
  };

  // Watch for changes to calculate cost automatically
          ...
          <div className="grid grid-rows-3 gap-6 col-span-3">
            ...
            <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 row-span-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-700">Invoice Attachment</label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  ...
                  <input
                    {...register("mrnNumber")}
                    className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.mrnNumber ? 'border-red-500' : 'border-gray-300'}`}
                    placeholder="Material receipt note #"
                  />
                  {errors.mrnNumber && <p className="text-red-500 text-xs mt-1">{errors.mrnNumber.message as string}</p>}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    {...register("ginNumber")}
                    className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.ginNumber ? 'border-red-500' : 'border-gray-300'}`}
                    placeholder="Goods issue note #"
                  />
                  {errors.ginNumber && <p className="text-red-500 text-xs mt-1">{errors.ginNumber.message as string}</p>}
                </div>
